...
CVPlot::CVPlot(view* View, const sf::Vector2f& position, const float& width, const float& height,
        ...
        axisLabels.emplace_back(View, sf::Vector2f(plotBounds.left + plotBounds.width/2,
                                ...
                                textEntry("x Label", fontName, 17*fontScaling,
                                          ALIGN_CENTER_MIDLINE, borderColor, sf::Text::Style::Bold),
                                sf::Color::Transparent, sf::Color::Transparent, 0.0f, CV_OBJ_ANIM_SLIDE);
        axisLabels.emplace_back(View, sf::Vector2f(plotBounds.left - plotBounds.width*0.4f,
...
CVPlot::Dataset::Dataset(const std::vector<std::vector<float>>& matrix, const std::vector<unsigned int>& indices,
    ...
    indices({indices}),
    name(name),
    color(plot->plotColor(plot->datasets.size())){
    ...
    while(plot->scaleAutoPadding.size() < matrix.size()) plot->scaleAutoPadding.emplace_back(false);
...
void CVPlot::axis::updateRenderLabels(){
        ...
        case DATA_TYPE_VERBAL:{
            ...
            for(auto& set : plot->datasets){
                for(auto& label : set.labels[dimension]){
                    if(!anyEqual(label, labels)) labels.push_back(label);
                }
            }
            break;
        }
    ...
    for(auto& text : renderLabels){
        ...
        text.setColor(textInfo.textColor);
        text.setCharacterSize(textInfo.fontSize*pow(plot->areaScale(), (1.0f/3)));
...
bool CVPlot::draw(){
    ...
    for(auto& Axis : plotAxes){ // Draw Axes
        View->viewPort->draw(Axis.line);
        for(auto& tick : Axis.lineTicks){
            View->viewPort->draw(tick);
        }
...
bool CVPlot::update(CVEvent& event, const sf::Vector2f& mousePos){
    ...
    while(dataAddRequests.size() > 0){
        ...
        while(plotColors.size() < datasets.size() + 1) plotColors.emplace_back(plotColors.front());

        if(dataAddRequests.front().labels.size() == 0) datasets.emplace_back(dataAddRequests.front().data, dataAddRequests.front().indices.front(),
    ...
    if(event.LMBhold && bounds.contains(mousePos) && event.viewHasFocus && event.captureMouse()){
        if(sf::Keyboard::isKeyPressed(sf::Keyboard::LShift) ||
            sf::Keyboard::isKeyPressed(sf::Keyboard::RShift)){
                if(!event.isCaptured(*this)){
                    if(mousePos.x > bounds.left + bounds.width*0.95f){
                        event.mouse_capture(*this);
                    }
                    else if(mousePos.x < bounds.left + bounds.width*0.05f){
                        event.mouse_capture(*this);
                    }
                    else if(mousePos.y < bounds.top + bounds.height*0.05f){
                        event.mouse_capture(*this);
                    }
                    else if(mousePos.y > bounds.top + bounds.height*0.95f){
                        event.mouse_capture(*this);
                    }
                    else move(mousePos - event.lastFrameMousePosition);
                }
        }
    ...
    if((sf::Keyboard::isKeyPressed(sf::Keyboard::LShift) ||
            sf::Keyboard::isKeyPressed(sf::Keyboard::RShift)) && event.isCaptured(*this)){
...
CVScatterPlot::CVScatterPlot(view* View, const sf::Vector2f& position, const float& width, const float& height,
    ...
    addAxis(NAN, DIMENSION_X, DATA_TYPE_NUMERIC, 5.0f, 12.0f, borderColor, CV_PLOT_AXIS_FONT_SIZE*fontScaling);
    addAxis(NAN, DIMENSION_Y, DATA_TYPE_NUMERIC, 5.0f, 12.0f, borderColor, CV_PLOT_AXIS_FONT_SIZE*fontScaling);
        ...
        if(updateState & CV_PLOT_UPDATE_DRAW){
            ...
            if(regressionType){
                ...
                for(auto& set : datasets){
                    ...
                    plotText.emplace_back("R  = " + strDigits(regression.back().coefficient, 4), *plotFont, 13*fontScaling*pow(areaScale(), 1.0f/3));
...
CVCategoryPlot::CVCategoryPlot(view* View, const sf::Vector2f& position, const float& width, const float& height,
           const sf::Color& fillColor, const sf::Color& borderColor, const sf::Color& plotColor,
           const float& borderWidth, const std::string& plotSprite,
           const std::string& fontName):
               CVPlot(View, position, width, height, fillColor, borderColor, plotColor, borderWidth,
    ...
    addAxis(NAN, DIMENSION_X, DATA_TYPE_VERBAL, 5.0f, 0.0f, borderColor, CV_PLOT_AXIS_FONT_SIZE*fontScaling);
    addAxis(NAN, DIMENSION_Y, DATA_TYPE_NUMERIC, 5.0f, 0.0f, borderColor, CV_PLOT_AXIS_FONT_SIZE*fontScaling);
                ...
                if(updateState & CV_PLOT_UPDATE_DRAW){
                    ...
                    if(showQ1 || showMedian || showQ3){
                        for(size_t i = 0; i < L; ++i){
                            ...
                            std::vector<float> labelData = set.getLabelledData(xAxis().labels[i], DIMY);
...
CVLinePlot::CVLinePlot(view* View, const sf::Vector2f& position, const float& width, const float& height,
           const sf::Color& fillColor, const sf::Color& borderColor, const sf::Color& plotColor,
           const float& borderWidth, const std::string& plotSprite,
    ...
    addAxis(NAN, DIMENSION_X, DATA_TYPE_NUMERIC, 5.0f, 0.0f, borderColor, CV_PLOT_AXIS_FONT_SIZE*fontScaling);
    addAxis(NAN, DIMENSION_Y, DATA_TYPE_NUMERIC, 5.0f, 0.0f, borderColor, CV_PLOT_AXIS_FONT_SIZE*fontScaling);
    ...
    if(framesLastChange < 5){
        ...
        if(updateState & CV_PLOT_UPDATE_DRAW){
            ...
            for(auto& set : datasets){
                ...
                for(auto& point : set.points){
                    if(point.visible) xDataValues.push_back(point.x());
                ...
                for(size_t i = 1; i < L; ++i){
                    switch(connectionType){
                        case CV_LINE_CONN_STEP:{
                            ...
                            if(cornerPos.y > set.points[orderedI[i-1]].sprite.getPosition().y){
                                backLines.emplace_back(sf::Vector2f(lineWidth,
                                                                    cornerPos.y - set.points[orderedI[i-1]].sprite.getPosition().y));
                                backLines.back().setOrigin(lineWidth/2, lineWidth/2);
                                backLines.back().setPosition(set.points[orderedI[i-1]].sprite.getPosition());
                            }
                            else{
                                backLines.emplace_back(sf::Vector2f(lineWidth,
                                                                    set.points[orderedI[i-1]].sprite.getPosition().y - cornerPos.y));
                                backLines.back().setOrigin(lineWidth/2, lineWidth/2);
                                backLines.back().setPosition(cornerPos);
                            }

                            if(set.points[orderedI[i-1]].selected && set.points[orderedI[i]].selected)
                                backLines.back().setFillColor(highlightColor);
                            else backLines.back().setFillColor(set.color);

                            backLines.emplace_back(sf::Vector2f(set.points[orderedI[i]].sprite.getPosition().x - cornerPos.x + lineWidth,
                                                   lineWidth));
                        ...
                        default:{
                            ...
                            backLines.back().setFillColor(set.color);
                            break;
                        }
                    }
        ...
        if(event.LMBhold){
            ...
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::LShift) ||
               sf::Keyboard::isKeyPressed(sf::Keyboard::RShift)){
                ...
                if(!resize){
                    for(auto& plot : selectedPlots){
                        if(plot->getBounds().contains(mousePos)) continue;
                        plot->move(mousePos - event.lastFrameMousePosition);
                    }
                }
            }
            else if(event.LMBholdFrames == 1){
            ...
            for(auto& plot : selectedPlots){
                ...
                highlightBlocks.back().setOutlineColor(highlightColor);
...
bool CVPlotPanel::draw(){
    if(!CVTextBox::draw()) return false;

    drawTarget->draw(logo);

    for(auto& block : highlightBlocks){
        drawTarget->draw(block);
    }
