...
CVMapPanel::CVMapPanel(CVView* parentView,
                       const string& panelTag,
                       const sf::Color& backgroundColor,
                       const sf::Vector2f& size,
                       const bool& bFitToWindow,
                       const sf::Vector2f& position):
    CVBasicViewPanel(parentView,
                     panelTag,
                     backgroundColor,
                     size,
                     bFitToWindow,
...
{
    return string(displayText[TextEntryIndex].getString());
}
...
void CVMapPanel::setZoneColor(const string& tag, const sf::Color& newColor)
{
    for(auto& zone : zones)
    {
...
bool CVMapPanel::zoneExists(const string& tag) const
{
    for(auto& zone : zones)
    {
...
void CVMapPanel::addZone(const string& name, const sf::Color& newColor,
                         const sf::Rect<int>& bounds)
{
    for(auto& zone : zones)
    {
...
void CVMapPanel::removeZone(const string& name)
{
...
void CVMapPanel::updateZone(const string& name, const sf::Vector2i& newCoords)
{
    for(auto& zone : zones)
    {
        if(zone.name == name)
...
void CVMapPanel::updateZone(const string& name, const sf::Rect<int>& newCoords)
{
    for(auto& zone : zones)
    {
        if(zone.name == name)
...
void CVMapPanel::setZoneImage(const string& tag, const sf::Texture& img)
{
    for(auto& zone : zones)
    {
        if(zone.name == tag)
