...
protected:
    ...
    hyperC::TimePoint                  end_time;
    hyperC::TimePoint                  frequency;

    std::vector<hyperC::TimePoint>      reminders;

    hyperC::time_scale scale;
    sf::Color color_code;
...
public:
    ...
    CVISION_API void set_repeat(const hyperC::TimePoint& interval);
    CVISION_API void clear_repeat();
    CVISION_API const hyperC::TimePoint& repeat_frequency() const;

    CVISION_API const hyperC::TimePoint& start() const;
    CVISION_API const hyperC::TimePoint& end() const;
    CVISION_API hyperC::TimePoint getDuration() const;
    CVISION_API std::string getDurationStr() const; // Get a string of the duration of this event

    CVISION_API void setStart(const hyperC::TimePoint& newStart);
    CVISION_API void setEnd(const hyperC::TimePoint& newEnd);

    CVISION_API const hyperC::time_scale& getScale() const;
    CVISION_API void setScale(const hyperC::time_scale& newScale);

    CVISION_API void addReminder(const hyperC::TimePoint& time_advance);    // Set a reminder time for this record
    CVISION_API bool flagReminder(const hyperC::TimePoint& time);           // Check if it's time to remind and if so, turn off the flag
    ...
    CVISION_API bool check_overlap(const hyperC::TimePoint& time) const;
    CVISION_API bool check_overlap(const CVCalendarRecord& other) const;

    CVISION_API hyperC::TimePoint duration() const;

    CVISION_API bool shift(const CVCalendarRecord& other, const hyperC::TimePoint& padding = MINUTES(15),
               const bool& forward = true);
    CVISION_API void shift(const hyperC::TimePoint& distance);
    ...
    CVISION_API CVCalendarRecord(const std::string& newInfo,
           const hyperC::TimePoint& start,
           const hyperC::TimePoint& end,
           const hyperC::time_scale& scale,
...
public:
    ...
    CVISION_API std::string getTimeTag(const hyperC::TimePoint& beginTime) const;
...
CVISION_API std::vector<CVCalendarRecord*> check_availability(CVSchedule& records,
                        const hyperC::TimePoint& begin_time,
                        const hyperC::TimePoint& end_time);

CVISION_API bool addRecord(CVSchedule& records,
               const std::string& info,
               const hyperC::TimePoint& time,
               const hyperC::TimePoint& end_time,
               const hyperC::time_scale& scale = hyperC::time_scale::any,
               const sf::Color& color_code = sf::Color(0,17,150));

CVISION_API bool changeRecord(CVSchedule& records,
                  const std::string& info,
                  const hyperC::TimePoint& time,
                  const hyperC::TimePoint& end_time); // Change an entry by name
CVISION_API bool changeRecord(CVSchedule& records,
                  const std::string& info,
                  const hyperC::TimePoint& new_start_time); // Change the start time of a record
CVISION_API bool changeRecords(CVSchedule& records,
                   const std::string& info,
                   const hyperC::TimePoint& time_change);    // Change groups of entries by common expression, shifted by time frame

CVISION_API bool changeRecord(CVSchedule& records,
                  const hyperC::TimePoint& old_time,    // Change a record by time span
                  const hyperC::TimePoint& new_time,
                  const hyperC::TimePoint& new_end_time,
                  const hyperC::time_scale& scale = hyperC::time_scale::any);

CVISION_API bool changeRecord(CVSchedule& records,
                  const hyperC::TimePoint& old_time,
                  const hyperC::TimePoint& new_time,
                  const hyperC::time_scale& scale = hyperC::time_scale::any);   // Change a record from old time to new time

CVISION_API bool removeRecord(CVSchedule& records,
                  const std::string& info);

CVISION_API bool removeRecord(CVSchedule& records,
                  const hyperC::TimePoint& start_time);
...
public:
    ...
    CVISION_API void setTimeScale(const hyperC::time_scale& newScale);

    CVISION_API void setTime(const hyperC::TimePoint& newTime);
    CVISION_API void moveTime(const hyperC::TimePoint& duration);
    ...
    CVISION_API void centerOnTime(const hyperC::TimePoint& newTime);

    CVISION_API bool removeRecord(const std::string& info);
    CVISION_API bool removeRecord(const hyperC::TimePoint& time);
    ...
    CVISION_API bool changeRecord(const hyperC::TimePoint& old_time,    // Change a record by time span
                      const hyperC::TimePoint& new_time,
                      const hyperC::TimePoint& new_end_time,
                      const hyperC::time_scale& scale);
